...
def model_fit(
    ...
    for e in range(epochs):
        ...
        for b, batch in enumerate(epoch_bar):
            ...
            # backward pass
            # print("AAAAAAAAAA", y_pred.shape, y_train.shape)
        ...
        # Early stopping
        # if the loss does not decrease for 3 consecutive epochs, stop training
        if e > early_stopping:
            if np.all(np.diff(history["val_loss"][-3:]) >= 0):
...
def classification_acc(type="multi-class"):
    ...
    def binary_acc(y_pred, y):
        ...
        correct = (y_pred == y).sum().item() / y.shape[1]
        return correct
